...
//Delays
#define tempo_curva 10

//Velocidades
#define veloc_Frente 180
#define veloc_Reverso 180
...
if (distancia < 10 and distancia > 0) {
  ...
  if (contador > 12) {
       // Serial.println("obstaculo");
        ...
        while(true){
         frente();
      if (analogRead(extrema_Direita)<corteD and analogRead(extrema_Esquerda) > corteE) {
        ...
        if (green_L > 80 and green_L < 100 ) {
            ...
            while(true){ 
              ...
              if (analogRead(meio)>corte_meio){
               ...
               if (green_R > 80 and green_R < 100) {
            frente();
            delay(tempo_curva);
            while(true){ 
              girar_direita();
              Serial.println("direita no verde");
              if (analogRead(meio)>corte_meio){
                para();
                Serial.println("Virei no verde");
                break;
...
//if (analogRead(extrema_Direita) > corteD and analogRead(extrema_Esquerda) > corteE and analogRead(meio)> corte_meio ){
//para();
